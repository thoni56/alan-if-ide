...
/** A re-wrap, as the edit it would be: replace from `from` to the string's end. */
...
/**
 * How re-wrapping one string would change the document -- or undefined if it would
 * not change it at all.
 *
 * <p>The single answer to "what does Re-wrap String do here", so the command and the
 * lightbulb that offers it cannot drift apart: the offer appears exactly when the
 * command would do something, because it is the same question asked once.
 *
 * @param unit the editor's one indent step, spaces or a tab, as the author writes it
 */
export function rewrapPlan(text: string, spans: StringSpan[], span: StringSpan,
    width: number, tabSize: number, unit: string): Rewrapped | undefined {
    ...
    // A STRING THAT ENDS UP SPANNING LINES IS A BLOCK, so give it one. Decided from
    // the RESULT rather than the input: what matters is whether the author is about to
    // have prose hanging off the end of a keyword. Format Document will not do this --
    // moving text between lines is outside its contract -- which is exactly why it
    // belongs to the command you had to ask for.
    const shouldMove = wrapped.includes('\n')
        && inlineColumn > visualWidth(own, tabSize)
        && !lineOpensInsideAnotherString(text, spans, span.start);
    if (shouldMove) {
        // One level in from the line it is leaving, and then wrapped as a string that
        // owns its line -- which it now does.
    ...
    return wrapped === literal ? undefined : { from, text: wrapped };
}
