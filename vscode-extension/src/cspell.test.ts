...
import {
    ALL_DICTIONARIES, BUNDLED, CSPELL_EXTENSION, DICTIONARIES, NAMES_FILE,
    configFor, dictionariesFor, gitignoreFor, languageNames,
} from './cspell';
import { ALAN_PATTERNS, CODE_DICTIONARIES } from './spelling';

/**
 * The file an author ends up with, which is the only part of this feature they can
 * be harmed by: it lands in their game folder, next to their manuscript, and it is
 * where cSpell's own "Add to dictionary" keeps the words they decided on.
 */
...
function fresh(languages = ['en']): any {
    const config = configFor(undefined, languages);
    assert.equal(config.ok, true);
    return parse((config as { ok: true; text: string }).text);
}

test('a fresh config carries the rules the measurement proved', () => {
    const c = fresh();
    ...
    // The generated list is never the target of "Add to dictionary": a word added
    // there would vanish at the next rebuild, silently.
    assert.deepEqual(c.dictionaryDefinitions, [
        { name: 'alan-project-names', path: `./${NAMES_FILE}`, addWords: false }]);
...
test('the author\'s own file survives being merged into', () => {
    ...
    const merged = parse((configFor(theirs, ['it']) as any).text);
...
test('running it twice produces the same file, not two of everything', () => {
    const once = (configFor(undefined, ['en']) as any).text;
    const twice = (configFor(once, ['en']) as any).text;
    assert.equal(twice, once);

    // And the same holds when the languages change: ours are replaced, not stacked.
    const changed = parse((configFor(once, ['en', 'sv']) as any).text);
...
test('a pattern of the author\'s own is kept beside ours', () => {
    ...
    const merged = parse((configFor(theirs, ['en']) as any).text);
...
test('a file we cannot understand is refused, not overwritten', () => {
    ...
    const broken = configFor('{ "words": [ // a comment cSpell allows\n] }', ['en']);
    assert.equal(broken.ok, false);
    assert.match((broken as any).reason, /not valid JSON/);

    const wrongShape = configFor('{ "patterns": "all of them" }', ['en']);
    assert.equal(wrongShape.ok, false);
    assert.match((wrongShape as any).reason, /"patterns" is not a list/);

    assert.equal(configFor('[]', ['en']).ok, false, 'an array is not a config');
});

test('the word list is gitignored once, however the entry is written', () => {
    ...
    assert.ok(added.endsWith(`/${NAMES_FILE}\n`));

    // Already covered, in either spelling: leave the file alone.
    assert.equal(gitignoreFor(`build/\n/${NAMES_FILE}\n`), undefined);
    assert.equal(gitignoreFor(`${NAMES_FILE}\n`), undefined);
    // And adding it is idempotent, since the command doubles as the repair.
...
test('English is the one language that costs nothing, and the list says so', () => {
    ...
    assert.equal(DICTIONARIES.filter(d => d.extension === undefined).length, 0);
    assert.equal(ALL_DICTIONARIES.length, DICTIONARIES.length + 1);
});

test('every dictionary names a distinct language and a real extension', () => {
    const codes = ALL_DICTIONARIES.map(d => d.code);
    assert.equal(new Set(codes).size, codes.length, 'no duplicate language codes');
    const names = ALL_DICTIONARIES.map(d => d.name);
...
test('the languages are named back to the author the way they chose them', () => {
    ...
    assert.deepEqual(dictionariesFor(['en', 'it']).map(d => d.name), ['English', 'Italian']);
    assert.deepEqual(
        dictionariesFor(['en', 'it']).filter(d => d.extension !== undefined).map(d => d.code),
        ['it']);
});
